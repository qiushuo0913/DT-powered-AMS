...
FilePath: /DT_powered_AMS_code/main_repara.py
Description: conventional AMS (only using PT data)--- "AMS" in the simulation
'''
...
if __name__ == '__main__':
    ...
    def train_eta(t, model, eta, data_list, context_vector_t, num_data, Epoch):
        # model - GNN with rewrite repara
        # eta - hyperparameter network
        # data_list - PT graph data ([\hat{N}_t])/ DT graph data with number $S+1$
        # context_vector_t - context vector at time step t
        # num_data - number of data in one t (\hat{N}_t), 保证一个numdata就是一种c下面的数据这样我们控制一种数据对应一种c
        ...
        # scheduler = optim.lr_scheduler.StepLR(step_size=60, gamma=0.5)
    ...
    ### onlyPT
    def overall_test_eta_onlyPT(model, eta, data_list_test, context_vector_test, csis_test, num_data_train, num_data_test, indicator):
        Sumrate_average = []
        Sumrate_best = []
        ...
        for t in range (scenario_config.T):
            if indicator == 2:
                ...
                data_list_DT_S = []
                context_vector_list_DT_S = []

           
            #change for the multi-update
            if args.p == 0:
               data_list_t = data_list_PT_S
            else: 
                data_list_t = data_list_DT_S
            ...
            for i in range(args.tesing_number):
                ...
                sumrate, wmmse_rate = test_eta(model, eta, context_vector_test_i, data_list_test_i, csis_test_i, num_data_test, t)
                Sum.append(sumrate)
            ...
            # tags1 = {'NtPT_%d_onlyPT_seed%d_Epoch%d_S%d_shortlength_%d' % (scenario_config.train_layouts_PT, args.seed, args.Epoch, args.S, args.short_length): np.average(Sum)}
            ...
            eta = train_eta(t, model, eta, data_list_t, context_vector_t, num_data_train, Epoch=args.Epoch)
    ...
    def joint_test_GNN():
        ...
        model_repara = IGCNet_repara(scenario_config).to(scenario_config.device)
        reset_randomness(args.seed)
        overall_test_eta_onlyPT(model_repara, eta_network, data_list_test, context_vector_test, CSIs_test, num_data_train=args.Nt, num_data_test=50, indicator=2)
